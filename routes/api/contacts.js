...
router.post('/export', validateToken, function(req, res, next) {
  ...
  var qry;
  if (
    Array.isArray(req.body.campaign) ||
  ...
  Contact.find(
    ...
    function(err, contacts) {
      ...
      stringify(contacts, { header: true }).pipe(res);
    },
  );
});
...
router.post('/upload', validateToken, upload.single('file'), async function(
  req,
  res,
  next,
) {
    ...
    try {
      ...
      if (checkIfAlreadyExists) {
        return res.status(400).json({
          message:
            'A campaign with this name exist. Please enter a different campaign name',
        });
      } else {
        campaignId = await new Campaign({
          campaign,
          userId: mongoose.Types.ObjectId(userId),
        }).save();
      }
  ...
  global.socket.emit('import_status', {
    total: fileRows.length,
    skip: req.body.skipTraced,
  });
  var insertedCount = 0;
  var updatedCount = 0;
  var skippedCount = 0;
  ...
  if (req.body.skipTraced) {
    var promises = fileRows.map((row, index) => {
      if (index % 100 === 0) {
        global.socket.emit('import_status_progress', {
          total: fileRows.length,
          index,
        });
      }
      ...
      data['lastNameOne'] =
        row[`${headers['lastName']}`] || row['INPUT_LAST_NAME'];
      data['propertyAddress'] =
        row[`${headers['propertyAddress']}`] || row['INPUT_ADDRESS_LINE1'];

      let uniqueStr = data['propertyAddress'].concat(data['lastNameOne']);
      data['internal'] = uniqueStr.replace(/[^A-Z0-9]/gi, '');
    ...
    Promise.all(promises).then(function(results) {
      // console.log(results);
      global.socket.emit('import_status_success', {
        total: fileRows.length,
        insertedCount,
        updatedCount,
        skippedCount,
        index: fileRows.length,
      });
    ...
    var promises = fileRows.map((row, index) => {
      ...
      let data = {};
      data['lastNameOne'] =
        row[`${headers['lastName']}`] || row['OWNER 1 LAST NAME'];
      data['propertyAddress'] =
        row[`${headers['propertyAddress']}`] || row['SITUS STREET ADDRESS'];

      let uniqueStr = data['propertyAddress']
        ? data['propertyAddress'].concat(data['lastNameOne'])
        : '';
      data['internal'] = uniqueStr.replace(/[^A-Z0-9]/gi, '');
      data['userId'] = mongoose.Types.ObjectId(userId);
      // data['internal'] = row['MAILING_STREET_ADDRESS'] || row['MAILING STREET ADDRESS'];
      data['campaign'] = campaignId._id
        ? mongoose.Types.ObjectId(campaignId._id)
        : mongoose.Types.ObjectId(campaignId);
      data['firstNameOne'] =
        row[`${headers['firstName']}`] || row['OWNER 1 FIRST NAME'];
      ...
      data['mailingName'] =
        row['OWNER_MAILING_NAME'] || row['OWNER MAILING NAME'];
      data['occupancy'] = '';
      data['ownershipType'] = '';
      data['mailingAddress'] = row[`${headers['mailingAddress']}`] || '';
      data['mailingCity'] =
        row[`${headers['mailingCity']}`] || row['MAIL CITY'];
      data['mailingState'] =
        row[`${headers['mailingState']}`] || row['MAIL STATE'];
      data['mailingZipCode'] =
        row[`${headers['MAIL ZIP/ZIP 4']}`] || row['MAIL ZIP/ZIP+4'];
      data['mailingCounty'] = row['MAIL_COUNTRY'] || row['MAIL COUNTRY'];

      data['propertyCity'] =
        row[`${headers['propertyCity']}`] || row['SITUS CITY'];
      data['propertyState'] =
        row[`${headers['propertyState']}`] || row['SITUS STATE'];
      data['propertyZipCode'] =
        row[`${headers['propertyZip']}`] || row['SITUS ZIP CODE'];
      data['propertyCounty'] = '';
      data['apn'] =
        row[`${headers['APN - FORMATTED']}`] || row['APN - FORMATTED'];
      data['equityValue'] =
        row[`${headers['equityValue']}`] || row['EQUITY VALUE'];
      data['equityPercentage'] =
        row[`${headers['equityPercentage']}`] || row['EQUITY PERCENTAGE'];
      data['estimatedValue'] = '';
      data['recordingDateOT'] =
        row[`${headers['recordingDateOT']}`] || row['OT-RECORDING DATE'];
      data['salePriceOT'] = '';
      data['deedTypeOT'] =
        row[`${headers['deedTypeOT']}`] || row['OT-DEED TYPE'];
      data['recordingDateLMS'] = row[`${headers['LMS-RECORDING DATE']}`];
      data['salePriceLMS'] =
        row[`${headers['salePriceLMS']}`] || row['LMS-SALE PRICE'];
      data['deedTypeLMS'] = '';
      data['dateOfDeath'] = '';
      data['dateOfBirth'] = '';
      ...
      if (data['internal']) {
        return Contact.findOneAndUpdate(
          { internal: data['internal'] },
          data,
          { upsert: true, new: true, rawResult: true, useFindAndModify: true },
          function(err, row) {
            if (err) {
              return err;
            }
...
router.get('/getCounts', validateToken, async function(req, res, next) {
  var userId = req.decoded.id;
  try {
    var contactCount = await Contact.count({
      userId: mongoose.Types.ObjectId(userId),
    });
    ...
    return res
      .status(500)
      .json({ error: e, message: 'Error in getCounts api.' });
  }
...
router.post('/send_to_export', validateToken, async function(req, res, next) {
  ...
  let contacts = await Contact.find(
    { userId: mongoose.Types.ObjectId(userId), groupId },
    {
      firstNameOne: 1,
      lastNameOne: 1,
      mailingAddress: 1,
  ...
  fs.writeFile('uploads/export.csv', csv, 'utf8', async function(err) {
    if (err) {
      ...
      var mailOptions = {
        ...
        text:
          'Hi Admin, Please find the attached exported file for the skip traced',
        attachments: [
          {
