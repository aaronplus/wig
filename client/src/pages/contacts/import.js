...
class ImportContacts extends React.Component {
  ...
  onUploadFile = info => {
    ...
    if (status === 'done') {
      if (response && response.headers && response.headers.length) {
        ...
        if (skipTraced) {
          if (
            (headers.includes('INPUT_LAST_NAME') || headers.includes('LAST NAME')) &&
          ...
          (headers.includes('OWNER 1 LAST NAME') || headers.includes('LAST NAME')) &&
  ...
  render() {
    ...
    const modalData = Object.keys(mapSchema).map(item => {
      ...
      return (
        <div className="row">
          <Form.Item className="col-md-4 map-d">
            <Input value={headerKey} disabled name="mapTo" />
          </Form.Item>
          <Form.Item className="col-md-6 map-d">
            {form.getFieldDecorator(`${item}`, {
              initialValue:
                fileHeaders && fileHeaders.includes(selectedHeader) ? selectedHeader : 0,
          ...
          <Form {...formItemLayout} onSubmit={this.handleSubmit}>
            <Form.Item label="Import">
              {form.getFieldDecorator('import', {
                ...
                <Upload
                  ...
                  listType="csv"
                  accept="csv"
                  action="http://localhost:5000/api/contacts/uploadFile"
            ...
            <Form.Item>
              <Button
                ...
                disabled={!fileHeaders || !showImportButton || spinner}
              >
                Import
              </Button>
              <Button
